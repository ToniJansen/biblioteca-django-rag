"""Fix SQLite date formats used by analytics views.

Migration 0003 created `ano_mes_saida` with escaped percent signs, which makes
SQLite return the literal string "%Y-%m" instead of values such as "2026-05".
"""
...
SQL_CREATE = """
CREATE VIEW vw_analytics_emprestimos_obt AS
SELECT
    e.id                        AS emprestimo_id,
    e.data_saida,
    e.data_devolucao_prevista,
    e.data_devolucao_real,
    strftime(char(37) || 'Y-' || char(37) || 'm', e.data_saida) AS ano_mes_saida,
    CASE
...
CREATE VIEW vw_analytics_acervo_por_tipo AS
SELECT
    l.tipo_obra,
...
CREATE VIEW vw_analytics_circulacao_mensal AS
SELECT
    ano_mes_saida,
    tipo_obra,
    status,
...
CREATE VIEW vw_analytics_leitores_resumo AS
SELECT
    leitor_id,
    leitor_nome,
    leitor_email,
...
CREATE VIEW vw_analytics_ia_cobertura AS
SELECT
    l.id           AS livro_id,
    l.titulo,
    l.autor,
...
SQL_REVERSE = SQL_DROP + SQL_CREATE.replace(
    "strftime(char(37) || 'Y-' || char(37) || 'm', e.data_saida)",
    "strftime('%%Y-%%m', e.data_saida)",
)


class Migration(migrations.Migration):
    dependencies = [
        ('core', '0003_analytics_views'),
    ]

    operations = [
        migrations.RunSQL(sql=SQL_DROP + SQL_CREATE, reverse_sql=SQL_REVERSE),
    ]
